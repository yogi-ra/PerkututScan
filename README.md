...
- **Security Scan Workflow**: Optionally run additional security tools (Trivy, Bandit, Pip-Audit, Semgrep) via the `-s/--security-scan` flag for comprehensive vulnerability assessment.
...
- **Semgrep**: For security audit
- **Trivy**: For security audit
...
- `-s, --security-scan`: Enable security-scan workflow (runs Trivy, Bandit, Pip-Audit, Semgrep).
- `--tools LIST`: Comma-separated list of security tools to run (trivy,bandit,pip-audit,semgrep).
